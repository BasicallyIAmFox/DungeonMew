...
public final class HideDarknessCommand {
    // usage: hideDarkness (get|set (true|false))
    public static void register(String name, LiteralCommandNode<FabricClientCommandSource> rootNode) {
        ...
        var getNode = literal("get")
                .executes(HideDarknessCommand::getState)
                .build();
    ...
    private static int getState(CommandContext<FabricClientCommandSource> ctx) {
        sendStateMessage(ctx.getSource(), Features.HIDE_DARKNESS.getValue());

        return Command.SINGLE_SUCCESS;
    }

    private static int setState(CommandContext<FabricClientCommandSource> ctx, boolean value) {
        Features.HIDE_DARKNESS.bind(value);
    ...
    private static void sendStateMessage(FabricClientCommandSource source, boolean value) {
        if (value)
            source.sendFeedback(systemText(Text.translatable("dungeonmew.feature.hide_darkness.enabled")));
        else
            source.sendFeedback(systemText(Text.translatable("dungeonmew.feature.hide_darkness.disabled")));
    }
}
